...
from espnet.nets.viterbi_align_back import *
from espnet.nets.viterbi_align_batch import viterbi_align as viterbi_align_batch
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        #group.add_argument('--enc-adaptive', default=False, type=strtobool)
        #group.add_argument('--dec-adaptive', default=False, type=strtobool)
        group.add_argument('--predict', default=False, type=strtobool)
        group.add_argument('--ctc-predict', default=False, type=strtobool)
        group.add_argument('--ce-predict', default=False, type=strtobool)
        group.add_argument('--sample-prob', default=0.0, type=float)
        return parser
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.predict:
            ...
            self.predictor = torch.nn.Linear(args.adim, 2)
            self.pred_criterion = torch.nn.CrossEntropyLoss(weight=torch.Tensor([1.0, 5.0]),reduce=False)
            if args.ctc_predict:
                self.prectc = CTC(odim, args.adim, args.dropout_rate, ctc_type=args.ctc_type, reduce=True)
            elif args.ce_predict:
                self.prece = torch.nn.Linear(args.adim, args.odim)

        self.predict = args.predict
        self.ctc_predict = args.ctc_predict
        self.ce_predict = args.ce_predict
